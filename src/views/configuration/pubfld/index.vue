  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch">
      ...
      <el-form-item label="数据类型" prop="fldtype">
        <el-select v-model="queryParams.fldtype" placeholder="数据类型" @change="handleQuery" clearable size="small">
          <el-option
...
<!--        <el-button icon="el-icon-refresh" size="mini" @click="resetQuery">重置</el-button>-->
    ...
    >
      ...
      <el-table-column prop="pathbuf" width="150px" label="节点" :show-overflow-tooltip="true">
        <template scope="scope">
          <span v-if="rowId === scope.row.fmtPubfldPk">
            <el-input name="pathbuf" class="titleInput" :ref="'autoInput'+ scope.row.fmtPubfldPk" v-model="scope.row.pathbuf"/>
          </span>
          <span v-else>
            {{ scope.row.pathbuf }}
          </span>
      ...
      <el-table-column prop="fldlen" label="长度">
        <template scope="scope">
          <span v-if="rowId === scope.row.fmtPubfldPk">
            <el-input name="fldlen" v-model="scope.row.fldlen"/>
          </span>
          <span v-else>
            {{ scope.row.fldlen }}
          </span>
...
<!--              </el-dropdown-item>-->
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="报文名" prop="pubfldname">
              <el-input ref="formInput" v-model="form.pubfldname" placeholder="请输入报文名" :disabled="pubDisabled" @blur="pubfldnameLeave(form.pubfldname)"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" modal-append-to-body width="400px" append-to-body>
      <el-form ref="importForm" :model="importForm" :rules="rules">
        <el-form-item label="报文名" prop="pubfldname">
          <el-input ref="importPubfldname" v-model="importForm.pubfldname"></el-input>
        </el-form-item>
    ...
    <upload-excel v-if="uploadVisible" ref="uploadExcelRef"></upload-excel>
    <!--粘贴覆盖数据对话框-->
...
<script>
  import { listPubfld, addPubfld, delPubfld, updatePubfld, getPubfld, listPub, isExistPub, newPasteData, addPasteData, batchUpdatePid } from "@/api/configuration/pubfld";
  ...
  export default {
    name: "Pubfld",
    components: { Treeselect, IconSelect, uploadExcel },
    data() {
      return {
        // 遮罩层
        loading: true,
