<template>
  <div>
    <div>
      ...
      <div v-if="fileList6.length > 0" style="margin-right:300px;margin-top: 10px;">
        <el-upload
          disabled="true"
          :file-list="fileList6"
          action=""
      ...
      <div v-if="fileList7.length > 0" style="margin-right:300px;margin-top: 10px;">
        <el-upload
          disabled="true"
          :file-list="fileList7"
          action=""
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="140px">
      <el-form-item
        v-if="this.$store.getters.roles.indexOf('SX-JBG') !== -1 && showUpload && isApproval"
        label="银行承诺函(提交省级平台前上传)"
        prop="fileList8"
      >
        <el-upload
          :file-list="fileList8"
          ...
          :auto-upload="false"
        >
          <el-button slot="trigger" style="margin-right:10px;margin-top: 10px;" size="mini" type="primary">
            点击选择文件</el-button>
          <el-button
            style="margin-right:40px;margin-top: 10px;"
            size="mini"
            type="success"
            @click="submitUpload(8)"
          >确定上传</el-button>
        </el-upload>
      </el-form-item>
      <el-form-item
        v-if="this.$store.getters.roles.indexOf('SX-FXGLG') !== -1 && showUpload && isApproval"
        label="尽调报告(由风险管理岗上传)"
        prop="fileList9"
      >
        <span>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button
        v-if="this.$store.getters.roles.indexOf('SX-JBG') !== -1 && isApproval"
