        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAdd"
                    v-hasPermi="['zdlc:workflowZdlc:add']">新增
                </el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="danger" icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
                    v-hasPermi="['zdlc:workflowZdlc:del']">删除
                </el-button>
            </el-col>

            <el-col :span="1.5">
                <el-button type="warning" icon="el-icon-download" size="mini" @click="handleExport"
                    v-hasPermi="['zdlc:workflowZdlc:export']">导出
                </el-button>
            </el-col>
            <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>

        <el-table v-loading="loading" ref="multipleTable" :data="tableList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    <el-button v-if="'1' == scope.row.status || '3' == scope.row.status" size="mini" type="text"
                        icon="el-icon-edit" @click="handleUpdate(scope.row)">查看/修改
                    </el-button>
                    <el-button v-if="'1' == scope.row.status" size="mini" type="text" icon="el-icon-edit"
                        @click="handleSub(scope.row)">提交
                    </el-button>
                    <el-button
                        v-if="'1' == scope.row.status || '5' == scope.row.status || '4' == scope.row.status || '7' == scope.row.status"
                        size="mini" type="text" icon="el-icon-edit" @click="handleDelete(scope.row)">删除
                    </el-button>
                    <el-button v-if="'7' == scope.row.status" size="mini" type="text" icon="el-icon-edit"
                        @click="handleUpdate(scope.row)">重新提交
                    </el-button>

                    <el-button v-if="'1' != scope.row.status" size="mini" type="text" icon="el-icon-edit"
                        @click="historyFory(scope.row)">审批详情
                    </el-button>
        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
            ...
            <el-form ref="form" :model="form" :rules="rules" :isDrfat="isDraft" label-width="140px">
                <el-form-item label="借款企业情况" prop="fileList1">
                    <el-upload ref="upload1" :file-list="fileList1" action='' accept=".pdf"
                        :on-remove="(file, fileList) => { handleRemove(file, fileList, 1) }"
                        :on-change="(file, fileList) => { handleChange(file, fileList, 1) }" multiple
                        :on-preview="handlePreview" :with-credentials="true" :on-exceed="handleExceed"
                        :auto-upload="false">
                        <el-button size="small" type="primary">点击上传借款企业情况</el-button>
                    </el-upload>
                </el-form-item>
                <el-form-item label="转贷申请表" prop="fileList2">
                    <el-upload :file-list="fileList2" action='' accept=".pdf" :before-upload="beforeUpload"
                        :on-remove="(file, fileList) => { handleRemove(file, fileList, 2) }"
                        :on-change="(file, fileList) => { handleChange(file, fileList, 2) }" multiple
                        :on-preview="handlePreview" :with-credentials="true" :on-exceed="handleExceed"
                        :auto-upload="false">
                ...
                <el-form-item label="基础资料扫描原件" prop="fileList3">
                    <el-upload :file-list="fileList3" accept=".pdf" :before-upload="beforeUpload"
                        :on-remove="(file, fileList) => { handleRemove(file, fileList, 3) }" action=''
                        :on-change="(file, fileList) => { handleChange(file, fileList, 3) }" multiple
                        :on-preview="handlePreview" :with-credentials="true" :on-exceed="handleExceed"
                        :auto-upload="false">
                ...
                <el-form-item label="借款人背景资料" prop="fileList4">
                    <el-upload :file-list="fileList4" accept=".pdf" :before-upload="beforeUpload"
                        :on-remove="(file, fileList) => { handleRemove(file, fileList, 4) }" action=''
                        :on-change="(file, fileList) => { handleChange(file, fileList, 4) }" multiple
                        :on-preview="handlePreview" :with-credentials="true" :on-exceed="handleExceed"
                        :auto-upload="false">
                ...
                <el-form-item label="其他资料" prop="fileList5">
                    <el-upload :file-list="fileList5" accept=".pdf" :before-upload="beforeUpload"
                        :on-remove="(file, fileList) => { handleRemove(file, fileList, 5) }" action=''
                        :on-change="(file, fileList) => { handleChange(file, fileList, 5) }" multiple
                        :on-preview="handlePreview" :with-credentials="true" :on-exceed="handleExceed"
                        :auto-upload="false">
